...
| **Shared career (classic co-op)** | off | On = everyone shares one career/economy (classic co-op). Off (default) = each player keeps their **own** progression, money and licences. |
| **Fresh career (ignore saved)** | off | Start the session from a clean career instead of your saved one. |
| **Auto-grant my licenses to joining players** | off | Joiners inherit the host's held licences while enabled - handy so a new tester isn't blocked by locked jobs/regions. |
...
- LocoMP is an independent, fan-made project. It is **not** affiliated with, authorised or endorsed
  by **Altfuture s.r.l.**, the developers of Derail Valley.
