...
**Why it exists:** it lets you test multiplayer **solo**. You join it
...
By default the server runs a small synthetic board (Alpha/Bravo/Charlie/Delta, licence-free jobs).
`--config <file.lmpc>` loads a REAL career instead - actual yards, cargo economy, licence gates, route
...
**Watch a server train get driven:** run one bot that borrows and drives one of the server's
...
- **Player takeover of a server train is wire-level + bot-only so far.** A client can claim one
...
- **The default career board is a synthetic placeholder** (stations Alpha/Bravo/Charlie/Delta; jobs need
  no licence). `--config <file.lmpc>` now loads a real career (see above), but the tool that *exports* a
  `.lmpc` straight from a running game - actual yards, cargo economy, licence gates, route distances, like
...
- **Container packaging is here (`../../docker/`)** - a game-free two-stage image + compose
  (raw UDP, no reverse proxy; graceful SIGTERM save). Building the image and deploying it anywhere
  are manual, deliberate steps. Rate-limiting is still a later slice - friend-scale + local testing
  for now.
